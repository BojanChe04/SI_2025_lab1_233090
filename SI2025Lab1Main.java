...
class TaskManager {
    ...
    public List<Task> getMostUrgentTasks() {
        ...
        for (Task task : tasks) {
            if (!task.isCompleted() && task.getPriority() == Priority.HIGH) {
    ...
    public void markTaskCompleted(String name) {
        for (Task task : tasks) {
            if (task.getName().equals(name)) {
                task.complete(); // Fix: call 'complete()' instead of 'markAsCompleted()'
                break;
            }
        }
    }
...
public class SI2025Lab1Main {
    public static void main(String[] args) {
        ...
        // Optional: Call other methods (like sorting or filtering)
